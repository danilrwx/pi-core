...
5. Package the skill (run package_skill.ts)
...
```bash
./scripts/init_skill.ts <skill-name> --path <output-directory>
```
...
```bash
./scripts/package_skill.ts <path/to/skill-folder>
```

Optional output directory specification:

```bash
./scripts/package_skill.ts <path/to/skill-folder> ./dist
```
