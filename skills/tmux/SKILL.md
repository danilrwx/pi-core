...
```bash
SOCKET_DIR=${TMPDIR:-/tmp}/agentic-tmux-sockets  # well-known dir for all agent sockets
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/agentic.sock"                # keep agent sessions separate from your personal tmux
SESSION=agentic-python                           # slug-like names; avoid spaces
...
To monitor this session yourself:
  tmux -S "$SOCKET" attach -t agentic-lldb

Or to capture the output once:
  tmux -S "$SOCKET" capture-pane -p -J -t agentic-lldb:0.0 -S -200
```
...
- Agents MUST place tmux sockets under `CLAUDE_TMUX_SOCKET_DIR` (defaults to `${TMPDIR:-/tmp}/agentic-tmux-sockets`) and use `tmux -S "$SOCKET"` so we can enumerate/clean them. Create the dir first: `mkdir -p "$CLAUDE_TMUX_SOCKET_DIR"`.
- Default socket path to use unless you must isolate further: `SOCKET="$CLAUDE_TMUX_SOCKET_DIR/agentic.sock"`.

## Targeting panes and naming

- Target format: `{session}:{window}.{pane}`, defaults to `:0.0` if omitted. Keep names short (e.g., `agentic-py`, `agentic-gdb`).
...
- Scan all sockets under the shared directory: `./scripts/find-sessions.sh --all` (uses `CLAUDE_TMUX_SOCKET_DIR` or `${TMPDIR:-/tmp}/agentic-tmux-sockets`).
